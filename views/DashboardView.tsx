...
const DashboardView: React.FC = () => {
  ...
  const cards = [
    ...
    { label: 'ACTIVE TRACKS', value: stats.classCount, icon: BookOpen, bg: 'bg-amber-500/10', text: 'text-amber-500', delay: 'stagger-2' },
    { label: 'PENDING VERIFICATION', value: stats.pendingLecs, icon: Clock, bg: 'bg-rose-500/10', text: 'text-rose-500', delay: 'stagger-3' },
    { label: 'DISBURSED TOTAL', value: `₹${stats.totalPaidSalaries.toLocaleString()}`, icon: Wallet, bg: 'bg-emerald-500/10', text: 'text-emerald-500', delay: 'stagger-4' },
  ];
    ...
    <div className="space-y-8 animate-slide-up pb-20">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div className="flex flex-col gap-1.5">
          <h2 className="text-3xl md:text-5xl font-extrabold theme-text uppercase tracking-tighter">Operations Hub</h2>
          <div className="flex items-center gap-3">
             <div className="w-2.5 h-2.5 rounded-full bg-cyan-500 shadow-[0_0_8px_rgba(6,182,212,0.8)]"></div>
             <span className="text-[11px] font-bold theme-text-muted uppercase tracking-widest">Real-time Performance Metrics</span>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="theme-card rounded-[2.5rem] p-8 md:p-10 border shadow-sm flex flex-col justify-center items-center text-center animate-slide-up stagger-5">
           ...
           <h4 className="text-2xl font-black theme-text uppercase tracking-tight mb-2">{stats.classCount} Active Tracks</h4>
           <p className="text-xs font-bold theme-text-muted uppercase tracking-widest max-w-xs leading-relaxed">
             Managing compensation loads across {stats.teacherCount} faculty members and multiple JEE/Mains batches.
           </p>
        </div>
      </div>
    </div>
