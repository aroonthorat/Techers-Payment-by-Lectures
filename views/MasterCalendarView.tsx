...
const MasterCalendarView: React.FC = () => {
  ...
  return (
    <div className="space-y-8 animate-slide-up relative pb-32">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
        ...
        <div className="flex items-center gap-4 w-full md:w-auto">
          <button 
            ...
            className="p-4 theme-card rounded-2xl hover:theme-bg-primary hover:text-white transition-all shadow-sm active:scale-95"
          >
            <ChevronLeft className="w-6 h-6" />
          </button>
          <div className="flex-1 md:flex-none theme-card border-2 theme-border px-10 py-4 rounded-[1.5rem] flex items-center justify-center min-w-[240px] shadow-lg">
      ...
      <div className="theme-card rounded-[2rem] md:rounded-[3rem] shadow-2xl overflow-hidden border-2 theme-border">
        <div className="grid grid-cols-7 bg-[var(--bg-card)] border-b-2 theme-border">
        ...
        <div className="grid grid-cols-7 gap-px bg-[var(--border)]">
          {Array.from({ length: firstDay }).map((_, i) => (
            <div key={i} className="bg-[var(--bg-card)] opacity-30 h-24 md:h-44"></div>
          ))}
          {Array.from({ length: daysInMonth }).map((_, i) => {
            ...
            return (
              <div 
                ...
                className={`
                  bg-[var(--bg-card)] h-24 md:h-44 px-1.5 py-3 md:p-6 relative hover:bg-[var(--primary-light)]/40 cursor-pointer transition-all group overflow-hidden border-r border-b theme-border
      ...
      {selectedDayEvents && (
        <div className="fixed inset-0 bg-slate-900/90 z-[200] flex justify-end animate-in fade-in duration-300 backdrop-blur-sm">
          <div className="w-full max-w-xl theme-card h-full shadow-2xl animate-in slide-in-from-right duration-300 flex flex-col border-l theme-border overflow-hidden rounded-l-none md:rounded-l-[3rem]">
            <div className="p-6 md:p-10 border-b theme-border flex items-center justify-between bg-[var(--bg-card)]">
            ...
            <div className="flex-1 overflow-y-auto p-6 md:p-10 space-y-10 md:space-y-12 pb-40 custom-scrollbar bg-[var(--bg-main)]">
              <div className="space-y-4 md:space-y-6">
                ...
                {selectedDayEvents.attendance.length === 0 ? (
                   <div className="p-8 md:p-10 text-center theme-card rounded-[1.5rem] md:rounded-[2rem] border-2 border-dashed theme-border opacity-40">
                ...
                {selectedDayEvents.payments.length === 0 ? (
                  ...
                  <div className="grid gap-3 md:gap-4">
                    {selectedDayEvents.payments.map(p => {
                      ...
                      return (
                        <div key={p.id} className="p-6 md:p-8 bg-emerald-600 rounded-[1.5rem] md:rounded-[2.5rem] shadow-xl relative overflow-hidden text-white border border-white/10">
                          ...
                          <div className="flex justify-between items-end relative z-10 border-t border-emerald-500 pt-3 md:pt-4">
                            ...
                            <div className="text-[7px] md:text-[8px] font-black bg-emerald-900 text-emerald-100 px-3 py-1 md:px-4 md:py-1.5 rounded-full shadow-sm border border-emerald-400/30">VERIFIED TRANSACTION</div>
                          </div>
                        </div>
                      )
                    })}
                  ...
                  <div className="space-y-6 md:space-y-8 pl-3 md:pl-4 border-l-2 theme-border">
                    {selectedDayEvents.system.map(e => (
                      <div key={e.id} className="relative group">
                         <div className="absolute -left-[19px] md:-left-[21px] top-1 w-3 h-3 md:w-4 md:h-4 rounded-full theme-card border-2 theme-border group-hover:theme-bg-primary transition-colors"></div>
                         <div>
