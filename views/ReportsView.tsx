...
import { Download, FileText, Calendar, IndianRupee, User, ArrowDownRight } from 'lucide-react';
...
const ReportsView: React.FC<ReportsViewProps> = ({ forcedTeacherId }) => {
  const [payments, setPayments] = useState<Payment[]>([]);
  const [teachers, setTeachers] = useState<Teacher[]>([]);
  const [classes, setClasses] = useState<ClassType[]>([]);
  const [filterTeacher, setFilterTeacher] = useState(forcedTeacherId || '');
    ...
    <div className="space-y-8 animate-slide-up pb-32">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
        <div className="flex flex-col gap-1.5">
          <h2 className="text-3xl md:text-5xl font-extrabold theme-text uppercase tracking-tighter leading-none">Financial History</h2>
          <p className="text-xs md:text-sm font-bold theme-text-muted uppercase tracking-widest opacity-60">Verified disbursement logs & historical records</p>
        </div>
      ...
      <div className="grid grid-cols-1 gap-6">
        {filteredPayments.length === 0 ? (
          <div className="py-32 text-center bg-[var(--bg-card)] rounded-[3rem] border-2 border-dashed theme-border opacity-60">
          ...
          filteredPayments.sort((a,b) => new Date(b.datePaid).getTime() - new Date(a.datePaid).getTime()).map((p, idx) => (
            <div key={p.id} className="theme-card p-8 md:p-10 rounded-[2.5rem] border shadow-sm hover:shadow-xl transition-all animate-slide-up" style={{ animationDelay: `${idx * 0.05}s` }}>
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div className="flex items-center gap-6">
                  <div className="w-14 h-14 bg-[var(--primary-light)] theme-primary rounded-2xl flex items-center justify-center shrink-0 border theme-border">
                ...
                <div className="flex items-center gap-8 md:text-right">
                  <div className="hidden md:block">
                     <div className="text-[10px] font-black theme-text-muted uppercase tracking-widest mb-1 opacity-50">Gross Billing</div>
                  ...
                  {p.advanceDeduction > 0 && (
                    <div className="hidden md:block text-rose-500">
                      <div className="text-[10px] font-black uppercase tracking-widest mb-1 opacity-50">Advance Adj.</div>
                  ...
                  <div className="h-10 w-px theme-border border-l hidden md:block"></div>
                  
                  <div>
                    <div className="text-[10px] font-black theme-text-muted uppercase tracking-widest mb-1 opacity-50">Net Received</div>
