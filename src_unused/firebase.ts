...
export const dbService = {
  ...
  // --- DUAL STORAGE EXTENSIONS ---
  
  getAllData: async () => {
    if (db) {
      // If Firebase is active, we fetch snapshot of everything
  ...
  restoreData: async (data: any) => {
    if (db) {
      // Restore to Firebase (Destructive!)
      const batch = writeBatch(db);
      for (const col of Object.keys(data)) {
        if (!Array.isArray(data[col])) continue;
        // First delete existing (Simulated by just overwriting IDs, but realistically we should clear)
        // For safety in this demo, we'll just upscale
        for (const item of data[col]) {
          const { id, ...rest } = item;
          if (id) batch.set(doc(db, col, id), rest);
        }
      }
      await batch.commit();
    }
    // Always update local storage as cache/fallback
    saveLocalData(data);
  },
